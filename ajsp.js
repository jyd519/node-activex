...
global.Enumerator = function (arr) {
  ...
  this.moveNext = function () {
    this.nextItem = this.enum.Next(1);
    return this.atEnd();
  };
...
*/
    ...
    eval(
      "ArgF = function (" +
        argArr.join(",") +
        ") { return WScript.__Args[arguments[0]];  }"
    );
    ArgF.item = function (i) {
      return WScript.__Args[i];
    };
    ArgF.toString = function () {
      return WScript.__Args.join(" ");
    };
    ArgF.Count = function () {
      return WScript.__Args.length;
    };
...
// protocol line
console.log("AJSP/1.0 JSON UTF-8");
