...
#define NODE_DEBUG_MSG(msg)                                                                                            \
  {                                                                                                                    \
    printf(NODE_DEBUG_PREFIX "%s", msg);                                                                               \
    std::cout << std::endl;                                                                                            \
  }
#define NODE_DEBUG_FMT(msg, arg)                                                                                       \
  {                                                                                                                    \
    std::cout << NODE_DEBUG_PREFIX;                                                                                    \
    printf(msg, arg);                                                                                                  \
    std::cout << std::endl;                                                                                            \
  }
...
inline Local<String> v8str(Isolate *isolate, const wchar_t *text) { return v8str(isolate, (const uint16_t *)text); }
...
public:
  inline CComVariant() { memset((VARIANT *)this, 0, sizeof(VARIANT)); }
  ...
  inline HRESULT CopyTo(VARIANT *dst) { return VariantCopy(dst, this); }
  ...
  inline HRESULT ArrayGet(LONG index, CComVariant &var) {
    ...
    if (SUCCEEDED(hr) && vart != VT_VARIANT)
      var.vt = vart;
    return hr;
  }
  template <typename T> inline T *ArrayGet(ULONG index = 0) { return ((T *)parray->pvData) + index; }
...
public:
  inline CComException() { memset((EXCEPINFO *)this, 0, sizeof(EXCEPINFO)); }
  inline ~CComException() { Clear(true); }
...
template <typename T = IUnknown> class CComPtr {
public:
  T *p;
  inline CComPtr() : p(0) {}
  inline CComPtr(T *_p) : p(0) { Attach(_p); }
  inline CComPtr(const CComPtr<T> &ptr) : p(0) {
    if (ptr.p)
      Attach(ptr.p);
  }
  inline ~CComPtr() { Release(); }

  inline void Attach(T *_p) {
    Release();
    p = _p;
    if (p)
      p->AddRef();
...
Local<String> GetWin32ErrorMessage(Isolate *isolate, HRESULT hrcode, LPCOLESTR msg, LPCOLESTR msg2 = 0,
                                   LPCOLESTR desc = 0);
...
inline Local<Value> DispError(Isolate *isolate, HRESULT hrcode, LPCOLESTR id = 0, LPCOLESTR msg = 0,
  ...
  if (except) {
    if (except->scode != 0)
      obj->Set(ctx, v8str(isolate, "code"), Integer::New(isolate, except->scode));
    else if (except->wCode != 0)
      obj->Set(ctx, v8str(isolate, "code"), Integer::New(isolate, except->wCode));
    if (except->bstrSource != 0)
      obj->Set(ctx, v8str(isolate, "source"), v8str(isolate, except->bstrSource));
...
inline Local<Value> DispErrorNull(Isolate *isolate) { return Exception::TypeError(v8str(isolate, "DispNull")); }

inline Local<Value> DispErrorInvalid(Isolate *isolate) { return Exception::TypeError(v8str(isolate, "DispInvalid")); }

inline Local<Value> TypeError(Isolate *isolate, const char *msg) { return Exception::TypeError(v8str(isolate, msg)); }

inline Local<Value> InvalidArgumentsError(Isolate *isolate) {
  return Exception::TypeError(v8str(isolate, "Invalid arguments"));
}

inline Local<Value> Error(Isolate *isolate, const char *msg) { return Exception::Error(v8str(isolate, msg)); }

//-------------------------------------------------------------------------------------------------------

class NodeObject : public ObjectWrap {
public:
  template <typename T> static inline T *Unwrap(Local<Object> handle) {
...
inline HRESULT DispInvoke(IDispatch *disp, DISPID dispid, UINT argcnt = 0, VARIANT *args = 0, VARIANT *ret = 0,
                          WORD flags = DISPATCH_METHOD, EXCEPINFO *except = 0) {
  DISPPARAMS params = {args, 0, argcnt, 0};
  DISPID dispidNamed = DISPID_PROPERTYPUT;
  if (flags == DISPATCH_PROPERTYPUT) { // It`s a magic
    params.cNamedArgs = 1;
    params.rgdispidNamedArgs = &dispidNamed;
    if (params.rgvarg && params.rgvarg->vt == VT_DISPATCH)
      flags = DISPATCH_PROPERTYPUTREF;
  }
  return disp->Invoke(dispid, IID_NULL, 0, flags, &params, ret, except, 0);
}

inline HRESULT DispInvoke(IDispatch *disp, LPOLESTR name, UINT argcnt = 0, VARIANT *args = 0, VARIANT *ret = 0,
                          WORD flags = DISPATCH_METHOD, DISPID *dispid = 0, EXCEPINFO *except = 0) {
  LPOLESTR names[] = {name};
  DISPID dispids[] = {0};
  HRESULT hrcode = disp->GetIDsOfNames(GUID_NULL, names, 1, 0, dispids);
  if SUCCEEDED (hrcode)
    hrcode = DispInvoke(disp, dispids[0], argcnt, args, ret, flags, except);
  if (dispid)
    *dispid = dispids[0];
  return hrcode;
}
...
template <typename INTTYPE> inline INTTYPE Variant2Int(const VARIANT &v, const INTTYPE def) {
  ...
  case VT_VARIANT:
    if (v.pvarVal)
      return Variant2Int<INTTYPE>(*v.pvarVal, def);
  }
  VARIANT dst;
...
inline bool v8val2bool(Isolate *isolate, const Local<Value> &v, bool def) {
  ...
  if (v.IsEmpty())
    return def;
  if (v->IsBoolean())
    return NODE_BOOL(isolate, v);
  if (v->IsInt32())
    return v->Int32Value(ctx).FromMaybe(def ? 1 : 0) != 0;
  if (v->IsUint32())
    return v->Uint32Value(ctx).FromMaybe(def ? 1 : 0) != 0;
  return def;
}
...
public:
  ...
  inline bool IsDefault() {
    if (items.size() != 1)
      return false;
    auto &arg = items[0];
    if (arg.vt != VT_BSTR || arg.bstrVal == nullptr)
      return false;
...
template <typename IBASE = IUnknown> class UnknownImpl : public IBASE {
public:
  ...
  virtual ULONG __stdcall AddRef() { return InterlockedIncrement(&refcnt); }
...
public:
  ...
  inline DispObjectImpl(const Local<Object> &_obj, bool revargs = true, IID connIfIID = IID_NULL)
      : obj(Isolate::GetCurrent(), _obj), dispid_next(1), reverse_arguments(revargs), connIfIID(connIfIID) {}
  virtual ~DispObjectImpl() { obj.Reset(); }

  // IUnknown interface
  virtual HRESULT __stdcall QueryInterface(REFIID qiid, void **ppvObject) {
    if (qiid == this->connIfIID) {
      return UnknownImpl<IDispatch>::QueryInterface(IID_IDispatch, ppvObject);
    }
    // if (qiid == this->connIfIID) { *ppvObject = this; return S_OK; }
    if (qiid == CLSID_DispObjectImpl) {
      *ppvObject = this;
      return S_OK;
    }
